...
const DataRow: React.FC<{ transaction: Transaction }> = ({ transaction }) => {
  ...
  return (
    <ResponsiveGrid>
      ...
      <Text>{formatDollarAmount(transaction.amountUSD)}</Text>
      <Text>
...
const TransactionTable: React.FC<{
  ...
  useEffect(() => {
    if (transactions) {
      ...
      if (filteredTransactions.length % ITEMS_PER_PAGE === 0) {
        setMaxPage(Math.floor(filteredTransactions.length / ITEMS_PER_PAGE))
      } else {
        setMaxPage(Math.floor(filteredTransactions.length / ITEMS_PER_PAGE) + 1)
      }
    }
  ...
  return (
    <Wrapper>
      <Flex mb="16px">
        <Flex flexDirection={['column', 'row']}>
          <RadioGroup onClick={() => handleFilter(undefined)}>
            <Radio onChange={() => null} scale={isSm ? 'sm' : 'md'} checked={txFilter === undefined} />
            <Text ml="8px">{t('All')}</Text>
          </RadioGroup>

          <RadioGroup onClick={() => handleFilter(TransactionType.SWAP)}>
            <Radio onChange={() => null} scale={isSm ? 'sm' : 'md'} checked={txFilter === TransactionType.SWAP} />
        ...
        <Flex flexDirection={['column', 'row']}>
          <RadioGroup onClick={() => handleFilter(TransactionType.MINT)}>
            <Radio onChange={() => null} scale={isSm ? 'sm' : 'md'} checked={txFilter === TransactionType.MINT} />
            <Text ml="8px">{t('Adds')}</Text>
          </RadioGroup>

          <RadioGroup onClick={() => handleFilter(TransactionType.BURN)}>
            <Radio onChange={() => null} scale={isSm ? 'sm' : 'md'} checked={txFilter === TransactionType.BURN} />
      ...
      <AutoColumn gap="16px">
        <ResponsiveGrid>
          <Text color="secondary" bold onClick={() => handleSort(SORT_FIELD.amountUSD)} textTransform="uppercase">
            {t('Action')}
          </Text>
          <Text color="secondary" bold onClick={() => handleSort(SORT_FIELD.amountUSD)} textTransform="uppercase">
            {t('Total Value')} {arrow(SORT_FIELD.amountUSD)}
          </Text>
          <Text color="secondary" bold onClick={() => handleSort(SORT_FIELD.amountToken0)} textTransform="uppercase">
            {t('Token Amount')} {arrow(SORT_FIELD.amountToken0)}
          </Text>
          <Text color="secondary" bold onClick={() => handleSort(SORT_FIELD.amountToken1)} textTransform="uppercase">
            {t('Token Amount')} {arrow(SORT_FIELD.amountToken1)}
          </Text>
          <Text color="secondary" bold onClick={() => handleSort(SORT_FIELD.sender)} textTransform="uppercase">
            {t('Account')} {arrow(SORT_FIELD.sender)}
          </Text>
          <Text color="secondary" bold onClick={() => handleSort(SORT_FIELD.timestamp)} textTransform="uppercase">
